...
class CFileRotatHandlerConfig(CBaseHandlerConfig):
    ...
    def __str__(self):
        return "%s, filename:%s, mode:%s, encoding:%s, delay:%d, backupcount:%d" % (
        super(CFileRotatHandlerConfig, self).__str__(), \
...
class CRotatingFileHandlerConfig(CFileRotatHandlerConfig):
    def __init__(self, level='debug', formatter='simple', \
                 filename=DEFAULT_PATH, maxBytes=4 * 1024 * 1024 * 1024, mode='a', encoding=None, delay=0,
                 backupCount=0):
...
class CTimedRotatingFileHandlerConfig(CFileRotatHandlerConfig):
    def __init__(self, level='debug', formatter='simple', \
        ...
        super(CTimedRotatingFileHandlerConfig, self).__init__(level, formatter, filename, mode, encoding, delay,
                                                              backupCount)
        self._when = when
    ...
    def __str__(self):
        return "%s, when:%s, interval:%d" % (
        super(CTimedRotatingFileHandlerConfig, self).__str__(), self._when, self._interval)
...
class CNatureTimedRotatingFileHandlerConfig(CTimedRotatingFileHandlerConfig):
    def __init__(self, level='debug', formatter='simple', \
        ...
        super(CNatureTimedRotatingFileHandlerConfig, self).__init__(level, formatter, filename, when, interval, mode,
                                                                    backupCount, \
                                                                    encoding, delay, utc)
